...
def parse(linelist):
    ...
    metadata = re.split(
        "; ", replace_nested_quotes(url_escape_inside_quotes(linelist[8].rstrip(";")))
    )
    ...
    with open(input_gtf) as file:
        for line in file:
            ...
            if feature == "gene":
                ...
                transcript_name = default(
                    lookup("transcript_name", metadata), transcript_id
                )
                metadata["transcript_name"] = transcript_name
                transcript_type = default(
                    lookup("transcript_type", metadata), g2b[gene_id]
                )
