...
def makeGTF(header, inputData):
    ...
    for i in range(len(genes)):
        ...
        geneBiotype = [
            pos.split("=")[1] for pos in column9 if pos.startswith("gene_biotype")
        ][0]
        outGene = genes[i][0:8]
        outGene.append(
            'gene_id "'
        ...
        if len(geneRNAs) != 0:
            ...
            geneTranscript = [
                CDSrow for CDSrow in CDS if re.search(geneID + ";", CDSrow[8])
            ]
        ...
        if len(geneTranscript) != 0:
            for row in geneTranscript:
                ...
                if row[2] == "CDS":
                    transcriptName = [
                        pos.split("=")[1] for pos in tmp if pos.startswith("Name=")
                    ][0]
                else:
                    transcriptName = [
                        pos.split("=")[1] for pos in tmp if pos.startswith("gene=")
                    ][0]
                outTranscript.append(
                    'gene_id "'
                    + geneID
                    + '"; gene_name "'
                    + geneName
        ...
        if len(geneExons) != 0:
            for row in geneExons:
                ...
                tmp = row[8].split(";")
                transcriptID2 = [
                    pos.split("=")[1] for pos in tmp if pos.startswith("Parent=")
                ][0].split("-", 1)[1]
                outExon.append(
                    'gene_id "'
                    + geneID
