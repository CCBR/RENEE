...
config = {
    ...
    ".rnaseq": {
        ".default": {
            ".output_preference": [
                ...
                "percent_duplication",
                "percent_aligned",
                "inner_distance_maxima",
                "median_insert_size",
                "mean_insert_size",
                "mean_mapping_quality",
                "mean_coverage",
                "avg_aligned_read_length",
                "pct_mrna_bases",
                "pct_coding_bases",
                "pct_intronic_bases",
                "pct_utr_bases",
                ...
                "uni_vec_percent_aligned",
                "percent_antisense_strand",
                "percent_sense_strand",
                "median_tin",
                "flowcell_lanes",
            ]
        }
    ...
    "multiqc_rseqc_infer_experiment.txt": {
        ...
        "typecast": {"percent_sense_strand": float, "percent_antisense_strand": float},
        "scaling_factor": {
...
def args(argslist):
    ...
    *files, odir = argslist[1:]
    ...
    # Check to see if user provided input files to parse
        ...
        print(
            "\n{}Error: Failed to provide input files to parse!{}".format(
                *config[".error"]
            ),
            file=sys.stderr,
        )
...
def parsed(file, delimiter="\t"):
    ...
    with open(file, "r") as fh:
        ...
        for line in fh:
            ...
            )  # remove extensions from sample name
