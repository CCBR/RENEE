...
g <- ggplot(multiQC, aes(PC1, PC2, color = flowcell_lanes), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = percent_duplication), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = percent_aligned), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = pct_utr_bases), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = pct_intronic_bases), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = inner_distance_maxima), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_cv_coverage), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_3prime_bias), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_insert_size), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = gc_content), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = median_tin), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = pct_coding_bases), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = rRNA_percent_aligned), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
g <- ggplot(multiQC, aes(PC1, PC2, color = percent_antisense_strand), xlab) +
  geom_point(size = multiQC$flowcell_lanes) +
  theme_minimal() +
...
```{r hierarchical-correlation-matrix, dpi=300}
# Helper Function
reorder_cormat <- function(cormat) {
  # Use correlation between variables as distance for hierarchincal clustering
...
additional_remove <- names(numericQC) %in% c(
  "total_read_pairs", "mean_insert_size", "avg_aligned_read_length", "pct_mrna_bases",
...
# Remove lower triangle and reshape from wide to long format
...
if (params$annot) {
  cheatmap <- ComplexHeatmap::Heatmap(hm_data,
    col = colorRamp2(
      seq(-max(abs(pca_exp), na.rm = T), max(abs(pca_exp), na.rm = T), length.out = 20),
